...
void getSlotInfo ( FILE *fp, slotInfoP slot )
{
    ...
    while ( ( read = getline (&line, &len, fp) ) != -1 ) // getline extract each line in the file and store in len
    {
        ...
        for ( int i=0; i<len; i++ )
        {
            ...
            // pdsch code 
            if ( (line[i] == 'D' && line[i+1] == 'S' && line[0] == '(' )) 
            {
                
                    slot->shar[DS][lineDlNum1] = DS ;

                    //Harq ID
                    //printf("check=%d\n",i);
                    slot->pdsch[DS][lineDlNum1].harqId = atoi(&line[i+7]); // allocate i+7 w.r.t DS 
                    
                    // Num Rb
                    int k=14,j=0;    // If this changes then offset for copy needs to be adjusted 
                    int rbLen = 0; 
                    char *rb = ( char * ) calloc (rbLen, sizeof(char));
                    if ( NULL == rb )
                    {
                        printf("critical error\n");
                        return;
                    ...
                    while ( 1 )
                    {
                        rb[k-14] = line[i+k];           /** Copy SFN */
                        //printf("frame inside%d:%d:%d\n", atoi(frame),i,k);
                        k++;
                        j++;
                        if ( j == 3) break;
                   }
                   slot->rb_data[DS][lineDlNum1] = atoi(rb);     /** Convert it to int */
                  
                   // Rb Start
                    int m=10,l=0;    // If this changes then offset for copy needs to be adjusted 
                    char *rbsta = ( char * ) calloc (rbLen, sizeof(char));
                    if ( NULL == rbsta )
                    {
                        printf("critical error\n");
                        return;
                    ...
                    while ( 1 )
                    {
                        rbsta[m-10] = line[i+m];           /** Copy SFN */
                        //printf("frame inside%d:%d:%d\n", atoi(frame),i,k);
                        m++;
                        l++;
                        if ( l == 3) break;
                   }
                   slot->rb_start[DS][lineDlNum1] = atoi(rbsta);     /** Convert it to int */
                
                   //nsymbstart
                   slot->pdsch[DS][lineDlNum1].nSymbStart= atoi(&line[i+19]);

                  printf("PDSCH:DS_HARQ %d,Rb_start %d,Num_Rb %d,Nsymbolstart %d\n",slot->pdsch[DS][lineDlNum1].harqId,slot->rb_start[DS][lineDlNum1],slot->rb_data[DS][lineDlNum1],slot->pdsch[DS][lineDlNum1].nSymbStart);

                
                lineDlNum1++;
                //printf("finalUSLINE=%d\n", lineDlNum1);
            ...
            else if ( (line[i] == 'U' && line[i+1] == 'L' && line[0] == '(' )) 
            {
                ...
                printf("UL:frame:%d,slot:%d\n", slot->subFrameNo[UL][lineUlNum],slot->slotNo[UL][lineUlNum]); 

               
                free (frame);   
                lineUlNum++;      
            }
            else if ( (line[i] == 'U' && line[i+1] == 'S' && line[0] == '(' )) 
            {
                  slot->shar[US][lineUlNum1] = US ;
                 //  printf("\n%d %d\n",US,lineUlNum1);
                  //Harq_id
                 slot->pusch[US][lineUlNum1].harqId = atoi(&line[i+7]); // allocate i+7 w.r.t US 
                   ...
                   slot->rb_data[US][lineUlNum1] = atoi(rb); 
                   ...
                   slot->rb_start[US][lineUlNum1] = atoi(rbsta);     /** Convert it to int */
                   slot->pusch[US][lineUlNum1].nSymbStart= atoi(&line[i+19]);

                   printf("PUSCH:US_HARQ %d,Rb_start %d,Num_Rb %d,Nsymbolstart:%d\n",slot->pusch[US][lineUlNum1].harqId, slot->rb_start[US][lineUlNum1],slot->rb_data[US][lineUlNum1],slot->pusch[US][lineUlNum1].nSymbStart);
                lineUlNum1++;
               // printf("finalUSLINE=%d\n", lineUlNum1);
            }
...
printf("-------------------------------------------------------------------------------------------------\n");
    printf("| Direction | Frame | Slot | Channel | RB Start | Num RB | NSymbol Start | Harq ID | Shar ID | \n");
    printf("-------------------------------------------------------------------------------------------------\n");
    ...
    for (int i = 0; i < MAX_SLOT_IDX; i++)
    {
        if (slot->dir[DL][i] == DL)
        {
            printf("|    DL     | %5d | %4d |  PDSCH  |  %7d | %6d |    %11d |   %6d |   %6d |\n",
                   slot->subFrameNo[DL][i], slot->slotNo[DL][i],
                   slot->rb_start[DS][i], slot->rb_data[DS][i],
                   slot->pdsch[DS][i].nSymbStart, slot->pdsch[DS][i].harqId, slot->shar[DS][i]);
        }
    }
    ...
    for (int i = 0; i < MAX_SLOT_IDX; i++)
    {
        if (slot->dir[UL][i] == UL)
        {
            printf("|    UL     | %5d | %4d |  PUSCH  |  %7d | %6d |    %11d |   %6d |   %6d |\n",
                   slot->subFrameNo[UL][i], slot->slotNo[UL][i],
                   slot->rb_start[US][i], slot->rb_data[US][i],
                   slot->pusch[US][i].nSymbStart, slot->pusch[US][i].harqId, slot->shar[US][i]);
        }
    }
